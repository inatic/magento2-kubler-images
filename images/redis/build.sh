...
finish_rootfs_build()
{
    ...
    sed-or-die '^maxmemory .*' 'maxmemory 4GB' "${_EMERGE_ROOT}"/etc/redis/redis.conf
    sed-or-die '^# maxmemory-policy .*' 'maxmemory-policy volatile-lru' "${_EMERGE_ROOT}"/etc/redis/redis.conf
    copy_gcc_libs
}
